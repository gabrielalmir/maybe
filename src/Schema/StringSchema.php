...
final class StringSchema extends AbstractSchema
{
  ...
  public function parse($input): string
  {
    ...
    if ($this->minLength !== null && strlen($value) < $this->minLength) {
      throw new ValidationException(
        ValidationErrorBag::single(
          new ValidationError(
            '$',
            sprintf('String must have at least %d characters', $this->minLength),
            'string.min',
          ),
        ),
      );
    ...
    if ($this->maxLength !== null && strlen($value) > $this->maxLength) {
      throw new ValidationException(
        ValidationErrorBag::single(
          new ValidationError(
            '$',
            sprintf('String must have at most %d characters', $this->maxLength),
            'string.max',
          ),
        ),
      );
    ...
    if ($this->pattern !== null && preg_match($this->pattern, $value) !== 1) {
      throw new ValidationException(
        ValidationErrorBag::single(
          new ValidationError('$', 'String does not match expected format', 'string.pattern'),
        ),
      );
    }
