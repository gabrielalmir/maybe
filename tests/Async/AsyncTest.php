...
it('propagates worker exceptions as TaskFailedException', function (): void {
  ...
  expect(fn() => $future->resolve())->toThrow(TaskFailedException::class, 'boom');
});
...
it('cancels a running task', function (): void {
  ...
  expect(fn() => $future->resolve())->toThrow(CancelledException::class);
});

it('times out a task and throws TimeoutException', function (): void {
  $future = Async::run(
    [TestTasks::class, 'sleepAndReturnString'],
    [300000, 'late'],
    ['timeout' => 0.05],
  );

  expect(fn() => $future->resolve())->toThrow(TimeoutException::class);
});
...
it('cleans up temporary files after resolve', function (): void {
  ...
  $value = Async::run(
    [TestTasks::class, 'sleepAndReturnInt'],
    [0, 7],
    ['temp_dir' => $tempDir],
  )->resolve();
