...
$applyCoupon = static function (int $subtotalInCents, Option $couponCode): Result {
  return $couponCode->match(static function (string $code) use ($subtotalInCents): Result {
    $normalized = strtoupper(trim($code));

    if ($normalized === 'BEMVINDO10') {
      return Result::ok($subtotalInCents - (int) round($subtotalInCents * 0.1));
    }
...
$checkout = static function (int $subtotalInCents, ?string $couponCode, string $method) use (
  $applyCoupon,
  $authorizePayment
): Result {
  $coupon = Option::fromNullable($couponCode)
    ->map('trim')
    ->flatMap(
      static fn(string $value): Option => $value === '' ? Option::none() : Option::some($value),
    );
...
foreach (
  ...
  echo $checkout($order['subtotal'], $order['coupon'], $order['method'])->match(
    static fn(string $transactionId): string => sprintf(
      '%s: pedido confirmado (%s)',
      $customerName,
      $transactionId,
    ),
